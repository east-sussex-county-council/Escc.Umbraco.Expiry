...
$projectName = "Escc.Umbraco.Expiry.Notifier" 
...
TransformConfig "$appSourceFolder\app.example.config" "$destinationFolder\$projectName\$projectName.exe.config" "$transformsFolder\$projectName\app.release.config"
if (Test-Path "$transformsFolder\$projectName\app.$websiteName.config") {
	# Transform to temp file to avoid file locking problem
	TransformConfig "$destinationFolder\$projectName\$projectName.exe.config" "$destinationFolder\$projectName\$projectName.exe.temp.config" "$transformsFolder\$projectName\app.$websiteName.config"
...
Write-Host
Write-Host "Done. The notifier needs to be configured as a scheduled task, if you have not already done so." -ForegroundColor "Green"